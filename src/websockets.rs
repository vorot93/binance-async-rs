...
impl WebSockets {
    ...
    pub fn event_loop(&mut self) -> Result<()> {
        while let Some(ref mut socket) = self.socket {
            let msg: String = socket.0.read_message()?.into_text()?;
            ...
            if msg.find(OUTBOUND_ACCOUNT_INFO) != None {
                ...
                let trades: TradesEvent = from_str(&msg)?;
                ...
                self.market_handler.as_ref().map(|h| h.depth_orderbook_handler(&depth_orderbook));
            }
        }
        Ok(())
    }
