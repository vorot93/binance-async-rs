...
impl Binance {
    ...
    pub fn get_depth<I>(&self, symbol: &str, limit: I) -> Result<impl Future<Item = OrderBook, Error = Error>>
    where
        I: Into<Option<u64>>,
    {
    ...
    pub fn get_price(&self, symbol: &str) -> Result<impl Future<Item = f64, Error = Error>> {
        ...
        Ok(self.get_all_prices()?.and_then(move |Prices::AllPrices(prices)| {
            Ok(prices
                .into_iter()
    ...
    pub fn get_book_ticker(&self, symbol: &str) -> Result<impl Future<Item = Ticker, Error = Error>> {
        ...
        Ok(self
            ...
            .and_then(move |BookTickers::AllBookTickers(book_tickers)| Ok(book_tickers.into_iter().find(|obj| obj.symbol == symbol).ok_or(BinanceError::SymbolNotFound)?)))
    }
    ...
    pub fn get_klines<S3, S4, S5>(&self, symbol: &str, interval: &str, limit: S3, start_time: S4, end_time: S5) -> Result<impl Future<Item = KlineSummaries, Error = Error>>
    where
        S3: Into<Option<u16>>,
        S4: Into<Option<u64>>,
        S5: Into<Option<u64>>,
    {
        let mut params = vec![("symbol", symbol.to_string()), ("interval", interval.to_string())];
